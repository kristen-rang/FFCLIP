...
class Encoder4Editing(Module):
    def __init__(self, num_layers, mode='ir_se',stylegan_size=1024):
...
class ConcatLinear(nn.Module):
    def __init__(self, dim_in, dim_c):
        ...
        self._layer = nn.Linear(dim_in + dim_c, dim_in)

    def forward(self, attribute, x):
        '''

        :param attribute: B,512
        :param x: B,64/30
        :return: B,64/30
        '''

        x_context = torch.cat((x, attribute), dim=2)
        out = x + self._layer(x_context)
        return out
...
class attention_mapping(nn.Module):
    def __init__(self, in_dim=18):
        ...
        self.k_matrix = nn.Linear(1, 18, bias=False)
        ...
        self.layer_norm = nn.LayerNorm([512], elementwise_affine=False)

        self.dk = 512

    def forward(self, w, attribute):
        q = self.q_matrix(w) # B,18,512
        k = self.k_matrix(attribute.transpose(-2, -1)) # B,512,1 ->B,512,18
