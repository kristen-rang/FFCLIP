...
# dim = 1 tensor to create a kernel
# dim = 2 tensor as a kernel
def make_kernel(k):
...
# h*w -> h-k.shape[0] + 1 , w-k.shape[1] + 1
...
# mod + conv3x3 + norm std + noise bias + leaky relu
...
class Generator(nn.Module):
    def __init__(
        ...
        # default size=1024, log_size = 10, num_layers=17
        ...
        ##latent_avg##############
        # self.register_buffer('latent_avg', torch.zeros([self.style_dim]))
    ...
    def forward(
        ...
        if not input_is_latent and not input_is_stylespace:
            styles = [self.style(s) for s in styles]  # z->w [w1,...w18] w[B,512]

        # Update moving average of W.
        # if self.w_avg_beta is not None and self.training:
        #     with torch.autograd.profiler.record_function('update_w_avg'):
        #         self.latent_avg.copy_(self.mean_latent(self.n_latent).lerp(self.latent_avg, self.w_avg_beta))
        ...
        if truncation < 1 and not input_is_stylespace:
            style_t = []
            if not truncation_latent:
                truncation_latent = self.latent_avg.cuda()
            for style in styles:
                style_t.append(
        ...
        # 3. latent in broadcast by random chose w1,w2
        # latent in [B,18,512]
        else:
            latent = styles
...
if __name__ == '__main__':
    dic = {'style_dim': 512, 'n_mlp': 8, 'size': 1024, 'channel_multiplier': 2, 'blur_kernel': [1, 3, 3, 1],
           'lr_mlp': 0.01}

    g = Generator(**dic)
    e4e = Encoder4Editing(num_layers=50,stylegan_size=1024,mode='ir_se')

    e4e_ckpt = torch.load('../../pretrained_models/e4e_ffhq_encode.pt', map_location=torch.device('cpu'))  #########
    print(e4e_ckpt['opts'])

    e4e_state_dict = {k.split('.', 1)[-1]: v for k, v in e4e_ckpt['state_dict'].items() if k.split('.')[0]=='encoder'}
    e4e.load_state_dict(e4e_state_dict, strict=False)
    # e4e =e4e.eval()

    # ckpt = torch.load('./network-snapshot-025000.pt', map_location='cpu')
    ckpt = torch.load('../../pretrained_models/stylegan2-ffhq-config-f.pt', map_location='cpu')
    g.load_state_dict(ckpt['g_ema'], strict=False)

    g.latent_avg = ckpt['latent_avg']
    g.eval()
